...
str_sql_template = "
select 
  round(num/{{str_binning}}) as count_group
  , count(1) as num 
from 
  (
    SELECT 
...
group by 
  round(num/{{str_binning}}) 
order by 
  count(1) "
str_sql = whisker::whisker.render(str_sql_template, tags)
result <- dbGetQuery(sc, str_sql)
ggplot(result) + geom_col(aes(x=count_group, y=num), fill="darkorange") + theme_minimal() + xlim(x=c(0,30))
print(result)
```

```{r}
var_binning = 2
result <- order_products__prior %>%
  group_by(order_id) %>%summarise(n_products = n()) %>%
  group_by(count_group=floor(n_products/var_binning)) %>%summarise(num = n()) %>%
  collect()
ggplot(result) + geom_col(aes(x=count_group, y=num), fill="darkorange") + theme_minimal() + xlim(x=c(0,30))
print(result)
```
...
FROM orders_tbl
where user_id <=10
ORDER BY user_id" %>% dbGetQuery(sc, .)
...
, COALESCE(SUM(days_since_prior_order) OVER 
